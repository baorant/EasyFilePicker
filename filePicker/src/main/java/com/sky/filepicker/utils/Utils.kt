...
//        deleteDir(context.cacheDir)
     ...
     * 获取目录下的所有文件和目录
     */
    fun getAllFiles(path:String):ArrayList<String> {
        val file = File(path)
    ...
    private val impMap= hashMapOf<String, ArrayList<String>>().apply {
        put(Environment.getExternalStorageDirectory().absolutePath, arrayListOf(
    ...
    fun isImportantFile(parent:String, path:String):Boolean {
        val impFiles = impMap[parent]
        if (impFiles!=null && impFiles.contains(path)) {
            return true
        }
        return false
    }
